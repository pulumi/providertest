...
type DiffConfigLog struct {
	Request  rpc.DiffRequest
	Response rpc.DiffResponse
}

// BUG: will currently fail to unmarshal the response due to a enums not having JSON unmashalling implemented.
func (l *GrpcLog) DiffConfigs() ([]DiffConfigLog, error) {
	var typedEntries []DiffConfigLog
	for _, entry := range l.WhereMethod(DiffConfig) {
		var typedEntry DiffConfigLog
		err := unmarshalEntry(entry, &typedEntry.Request, &typedEntry.Response)
		if err != nil {
			return nil, err
		}
